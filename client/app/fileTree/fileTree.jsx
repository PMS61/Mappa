...
import { createFile, uploadFile } from "./utils/fileUtils";
...
export default function FileTree({ paths, room, repoName, addfile }) {
  ...
  useEffect(() => {
    if (paths.length > 0) {
      ...
      const temp = [
        { room_id: Date.now().toString(), path: `${repoName}/.hidden` },
      ];
      setLocalPaths(temp);
    }
  ...
  return (
    <>
      ...
      <CreateFileModal
        ...
        onSubmit={(fileName) =>
          createFile(room, currentPath, fileName, localPaths, setLocalPaths)
        }
      />
    </>
  );
